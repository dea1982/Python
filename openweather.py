...
class CityBase:
    ...
    def check_size(self):  # Возвращает TRUE если размер базы в интернете равен скаченной базе
...
class SqlBase:
    ...
    def create_sql(self, city_dict):
        ...
        if check is False:
            ...
            for city in city_dict:
                ...
                if country != '':
                    c.execute(
                        'INSERT INTO {} VALUES ({},  "{}", "{}")'.format(self._city_table, id_city, name, country))
            sql.commit()
            sql.close()
    ...
    def weather_data(self, data, id_city, name, temp, id_weather):
        ...
        if check:
            ...
            c.execute(
                'INSERT INTO weather VALUES ({},  {}, "{}", {}, {})'.format(data, id_city, name, temp, id_weather))
            data = time.ctime(data)
            result_print = 'Дата: {} |ID: {} |Город: {} |Темература: {} |ID_погоды: {}'.format(data, id_city, name,
                                                                                               temp, id_weather)
            sql.commit()
            sql.close()
            return result_print
    ...
    def history(self, id_city):
        ...
        if check:
            for request in check:
                ...
                result_print.append(
                    'Дата: {} |ID: {} |Город: {} |Темература: {} |ID_погоды: {}'.format(data, request[1], request[2],
                                                                                        request[3], request[4]))
            sql.close()
            return result_print
        else:
    ...
    def full_name(self):
        ...
        if os.path.isfile(os.getcwd() + self._name_long) is not True:
            try:
                ...
                with urllib.request.urlopen(self.url_name_country) as response, open(self._name_long,
                                                                                     'wb') as out_file:
                    shutil.copyfileobj(response, out_file)
            except Exception:
                print('Нет соединения с интернетом')
...
class Gui(tkinter.Tk):
    ...
    def start(self):
        ...
        for i in list_choice:
            ...
            url_weather = 'http://api.openweathermap.org/data/2.5/weather?id={}&units=metric&appid={}'.format(id_city,
                                                                                                              ID)
            try:
